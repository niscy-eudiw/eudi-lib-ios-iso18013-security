...
struct CertificateHandlingSwiftTests {
    ...
    func testReaderCertificateValidations() throws {
        let certData = try #require(
            try Data(contentsOf: Bundle.module.url(forResource: "owf_identity_credential_reader_cert", withExtension: "der")!)
        )
        ...
        #expect(!isValid, "Certificate validation should fail with no root certs given")
    ...
    func testCRLParsing() throws {
        ...
        #expect(!crl.revokedSerials.isEmpty, "CRL should contain revoked serials")
    }
}
