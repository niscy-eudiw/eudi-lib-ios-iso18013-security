...
public struct TrustedListValidator: @unchecked Sendable {
    let urls = TrustListUrls()
    let validator: CachedTrustValidator
    
    public init(urlConfig: [EtsiListProvider: String], useCaching: Bool = true, ttlHours: Double = 24) {
        urls.pidProviders    = urlConfig[EtsiListProvider.pid]
        urls.walletProviders = urlConfig[EtsiListProvider.wallet]
    ...
    static let DIGI: TrustedListValidator =
        TrustedListValidator(
            urlConfig: [
                .pid : DIGITTrustLists.pidProviders,
                .wallet: DIGITTrustLists.walletProviders,
                .wrpac: DIGITTrustLists.wrpacProviders,
                .mdl: DIGITTrustLists.mdlProviders
            ]
        )
    
    static let EUDIRef: TrustedListValidator =
        TrustedListValidator(
            urlConfig: [
                .pid : EUDIRefImplLists.pidProviders,
                .wallet: EUDIRefImplLists.walletProviders,
                .wrpac: EUDIRefImplLists.wrpacProviders,
                .wrprc: EUDIRefImplLists.wrprcProviders
            ]
        )
